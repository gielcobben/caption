...
(function() {
  ...
  ga_storage = new function() {
    ...
    var visitor_custom_vars = new LocalStorage(
      "ga_storage_visitor_custom_vars"
    );

    var c_session = 0;
    var custom_vars = visitor_custom_vars._get()
      ? JSON.parse(visitor_custom_vars._get())
      : ["dummy"];
    ...
    this._setLocale = function(lng, country) {
      lng = typeof lng === "string" && lng.match(/^[a-z][a-z]$/i)
        ? lng.toLowerCase()
        : "en";
      country = typeof country === "string" && country.match(/^[a-z][a-z]$/i)
        ? country.toLowerCase()
        : "us";
      utmul = lng + "-" + country;
    ...
    this._setCustomVar = function(index, name, value, opt_scope) {
      ...
      if (opt_scope === 1) {
        var vcv = visitor_custom_vars._get()
          ? JSON.parse(visitor_custom_vars._get())
          : ["dummy"];
        vcv[index] = params;
    ...
    this._trackPageview = function(path, title, source, medium, campaign) {
      ...
      if (custom_vars.length > 1) {
        ...
        for (var i = 1; i < custom_vars.length; i++) {
          if (custom_vars[i]) last_slot = i;
        }
        for (i = 1; i < custom_vars.length; i++) {
          if (custom_vars[i]) {
            var slotPrefix = "";
            if (!custom_vars[i - 1]) slotPrefix = i + "!";

            names += slotPrefix + custom_vars[i].name;
            values += slotPrefix + custom_vars[i].value;
            scopes +=
              slotPrefix +
              (custom_vars[i].scope == null ? 3 : custom_vars[i].scope);
